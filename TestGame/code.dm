...
<body>
	...
	verb/demo_filters()
		...
		if(length(src.filters))
			...
			var/selected = input("Pick a filter", "Choose a filter to apply (with demo settings)", null) as null|anything in list("alpha", "alpha-swap", "alpha-inverse", "alpha-both", "outline", "greyscale", "blur", "outline/grey", "grey/outline", "color", "all")
			if(isnull(selected))
				src.filters = null
